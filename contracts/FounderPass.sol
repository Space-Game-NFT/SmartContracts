...
contract FounderPass is Ownable, ERC721A, ReentrancyGuard {
    ...
    constructor(
        ...
        address _weth
    ) ERC721A("FounderPass", "FP", _maxBatchMintSize) {
        ...
        nonReentrant
    {
        ...
        require(
            availableClaimCount(msg.sender, merkleProof) >= numberOfTokens,
        ...
        if (isPublicSale) {
            _publicMintedTokens[msg.sender] = _publicMintedTokens[msg.sender]
                .add(numberOfTokens);
        } else {
            _preMintedTokens[msg.sender] = _preMintedTokens[msg.sender].add(
                numberOfTokens
            );
        }
    }
    ...
    function availableClaimCount(
        ...
        if (!isPublicSale) {
            ...
            return
                _preMintedTokens[account] >= maxCountPerAccountPre
                    ? 0
                    : maxCountPerAccountPre.sub(_preMintedTokens[account]);
        } else {
            return
                _publicMintedTokens[account] >= maxCountPerAccountPublic
                    ? 0
                    : maxCountPerAccountPublic.sub(
                        _publicMintedTokens[account]
                    );
        }
    }
    ...
    function setMaxCountPerClaim(uint256 _count) external onlyOwner {
