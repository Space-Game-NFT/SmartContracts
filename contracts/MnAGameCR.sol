    ...
    IMnAGame
{
    ...
    function canMint(address addr) external view returns (bool) {
        return
            ...
            (_commitRandoms[_pendingCommitId[addr]] > 0 ||
                address(seedGenerator) != address(0));
    }
     ...
     * commit was added to. */
    function mintCommit(uint256 amount, bool stake)
        external
        whenNotPaused
        nonReentrant
    {
    ...
    function reveal(address addr) internal {
        ...
        for (uint256 k = 0; k < commit.amount; k++) {
            ...
            if (
                recipient != addr &&
                address(spidox) != address(0) &&
                spidox.balanceOf(addr) > 0
            ) {
                // If the mint is going to be stolen, there's a 50% chance
                //  a alien will prefer a treasure chest over it
                if (seed & 1 == 1) {
                    spidox.safeTransferFrom(
                        addr,
                        recipient,
                        spidox.tokenOfOwnerByIndex(addr, 0),
                        ""
                    );
                    recipient = addr;
                }
    ...
    function setMaxOresCost(uint256 _amount)
        external
        requireContractsSet
        onlyOwner
    {
